...
export function secretIncident(value, extra = {}) {
  ...
  return {
    ...
    // EVERY occurrence, not just the first. An approval binds to this list, so
    // recording only hit[0] would let a second secret in the same text be
    // approved by a decision that never saw it. `matchFingerprint` is the hash of
    // the match, never the match itself.
    occurrences: hits.map((h) => ({
      field: extra.face ?? extra.field ?? null,
      rule: h.patternName,
      start: h.start,
      end: h.end,
      matchFingerprint: h.sha256,
    })),
    // Revision 1 by construction. A rewrite of this incident must bump it, so an
    // approval for the old revision cannot apply to the new findings.
    incidentRevision: 1,
    ...extra,
  };
}
