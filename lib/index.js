...
const zEvolveSettings = z.object({
  ...
  disposalMode: z.union([z.const('manual'), z.const('suggest')]).default('manual'),
...
async function _applyInner(ctx, cfg, dshHome) {
  ...
  const idleTrigger = createIdleTrigger({
    ...
    onIdle: () => {
      // Only the suggest tier surfaces proposals. Recompute is pure/read-only.
      if (cfg.disposalMode !== 'suggest') return;
      try {
      ...
      } catch (e) { ctx.logger?.warn?.(`[dsh-evolve] disposal suggest recompute failed: ${e?.message ?? e}`); }
    },
  });
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const res = applySkillMutation({
        action: 'crystallize', source: 'model-tool', skillsDir, archiveDir, ownerId,
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const res = applySkillMutation({
        action: 'refine', source: 'model-tool', skillsDir, archiveDir, ownerId,
        logger: ctx.logger ?? undefined,
        onSecretIncident: recordSecretIncident,
        payload: { name: skillName, tag: args.tag, records, body: distilled?.body },
      });
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const res = applySkillMutation({
        action: 'converge', source: 'model-tool', skillsDir, archiveDir, ownerId,
        logger: ctx.logger ?? undefined,
        onSecretIncident: recordSecretIncident,
        payload: {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const res = applySkillMutation({
        action: 'fold', source: 'model-tool', skillsDir, archiveDir, ownerId,
        logger: ctx.logger ?? undefined,
        onSecretIncident: recordSecretIncident,
        payload: { name: args.name, body: distilled.body },
      });
    ...
    async apply(sctx) {
      try {
        ...
        const base = {
          ...
          disposalMode: ['manual', 'suggest'].includes(cfg.disposalMode) ? cfg.disposalMode : 'manual',
