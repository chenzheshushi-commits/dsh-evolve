...
async function _applyInner(ctx, cfg, dshHome) {
  ...
  const mutationOrProposal = (action, payload, source = 'model-tool', receiptSessionId = null) => {
    ...
    const result = applySkillMutation({ action, source, skillsDir, archiveDir, ownerId,
      logger: ctx.logger ?? undefined, onSecretIncident: recordSecretIncident, config: cfg, payload });
    if (result.ok) for (const r of receipts) usageReceipts.consume(r.receiptId, `direct_${Date.now().toString(36)}`);
    return result;
  };
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const res = mutationOrProposal('crystallize', {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const res = mutationOrProposal('refine', { name: skillName, tag: args.tag, records, body: distilled?.body }, 'model-tool', exec?.agent?.session?.id);
  ...
  ctx.tools.register(defineTool({
    ...
    output: jsonOutput(),
    async execute(args) {
      const res = applySkillMutation({
        action: 'archive', source: 'model-tool', skillsDir, archiveDir, ownerId,
        logger: ctx.logger ?? undefined,
        onSecretIncident: recordSecretIncident, payload: { name: args.name },
      });
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      const res = applySkillMutation({
        action: 'restore', source: 'model-tool', skillsDir, archiveDir, ownerId,
        logger: ctx.logger ?? undefined,
        onSecretIncident: recordSecretIncident, payload: { name: args.name },
      });
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      const res = applySkillMutation({
        action: 'rollback', source: 'model-tool', skillsDir, archiveDir, ownerId,
        logger: ctx.logger ?? undefined,
        onSecretIncident: recordSecretIncident, payload: { name: args.name },
      });
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const res = mutationOrProposal('converge', {
      ...
      if (!res.ok) return { merged: false, reason: res.reason };
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const res = mutationOrProposal('fold', { name: args.name, body: distilled.body }, 'model-tool', exec?.agent?.session?.id);
      ...
      if (overArchive.length > 0) {
        if (cfg.curatorAutoArchive === true) {
          ...
          for (const s of overArchive) {
            const r = applySkillMutation({
              action: 'archive', source: 'turn-end-auto', skillsDir, archiveDir, ownerId,
              logger: ctx.logger ?? undefined,
        onSecretIncident: recordSecretIncident, payload: { name: s.name },
            });
  ...
  try {
    ...
    const skillOps = {
      ...
      archive: (name) => applySkillMutation({
        action: 'archive', source: 'web-prune', skillsDir, archiveDir, ownerId,
        logger: ctx.logger ?? undefined,
        onSecretIncident: recordSecretIncident, payload: { name },
      }),
      fold: (name) => {
        const skill = readEvolveSkill(skillsDir, name);
        if (!skill) return { error: 'not evolve-owned' };
        return applySkillMutation({
          action: 'fold', source: 'web-prune', skillsDir, archiveDir, ownerId,
          logger: ctx.logger ?? undefined,
        onSecretIncident: recordSecretIncident,
          payload: { name, body: bodyBeforeRefinements(skill.body ?? '') },
        });
      },
      converge: (names, into) => {
        const skills = names.map((n) => readEvolveSkill(skillsDir, n)).filter(Boolean);
        ...
        const res = applySkillMutation({
          action: 'converge', source: 'web-prune', skillsDir, archiveDir, ownerId,
          logger: ctx.logger ?? undefined,
        onSecretIncident: recordSecretIncident,
          payload: {
            name: skillName, tag: mergedTag, records: [], body,
            originals: skills.map((x) => x.name),
          },
        });
        return res.ok
          ? { converged: res.converged ?? skillName, from: names, archivedOriginals: res.archivedOriginals ?? [] }
          : { error: res.reason };
      },
    };
 ...
 */
function setupWebSettings(ctx, { cfg, store, skillStates, llm, prune, getSuggestedDisposal }) {
      ...
      try {
        const routes = makeEvolveRoutes({
          ...
          proposalService: {
            ...
            apply: async (id, opId = `apply_${Date.now().toString(36)}`) => {
              ...
              let final = { ok: false, reason: 'unknown failure' };
              try {
                ...
                final = applySkillMutation({
                  action: p.action, source: 'web-human', skillsDir, archiveDir, ownerId,
                  logger: ctx.logger ?? undefined, onSecretIncident: recordSecretIncident,
                  config: cfg, expectedBaseHashes: p.baseHashes,
                  payload: {
                    name: p.targetSkill, tag: p.tag, body: p.body,
                    description: p.description, records: p.meta?.records ?? [], originals: p.originals ?? [],
                  },
                });
                proposalStore.finalize(id, opId, final.ok ? 'applied' : 'failed', final);
                if (final.ok) {
                  reservations.consume(id);
                  if (Array.isArray(p.sourceIds) && p.sourceIds.length) await store.markCrystallized(p.sourceIds);
                } else reservations.release(id);
                return { ...final, status: final.ok ? 'applied' : 'failed', proposalId: id };
              } catch (e) {
