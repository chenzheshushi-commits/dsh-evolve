...
const zEvolveSettings = z.object({
  ...
  // UI/prompt language. `follow-host` reads the host's `locale` namespace; en/zh
  // override it. Default is follow-host, and when the host exposes nothing the
  // resolver falls back to English (issue #1: a global audience should not have to
  // opt out of Chinese). Existing installs are migrated to 'zh' below so an upgrade
  // never silently flips somebody's UI.
  language: z.union([z.const('follow-host'), z.const('en'), z.const('zh')]).default('follow-host'),
});
...
/**
 * UI/prompt language in force. Resolved from the host `locale` namespace and this
 * plugin's own `language` override; see lib/i18n.js.
 *
 * Module-level rather than threaded through every call site because the prompt anchors
 * are built in a dozen places and a missed parameter would silently emit the wrong
 * language on one of them. The settings child plugin updates it, and it must stay in
 * step with PROTOCOL_SECTION -- which names the anchors -- so both read from here.
 */
let uiLang = 'en';
/** Set by the settings child plugin once the host locale and override are known. */
function setUiLanguage(next) { uiLang = next; }
/** Current language, for the prompt builders below. */
function lang() { return uiLang; }
...
function renderInjection(hits, maxChars) {
  const lines = [t(lang(), 'anchor.recall.header')];
...
/**
 * The protocol text, as a function of language.
 *
 * It NAMES the two injected anchors, so it cannot be a constant: a fixed Chinese
 * 「相关记忆」 here plus an English header at injection time tells the model to look for
 * something it never receives. scripts/schema/i18n.test.mjs asserts the pairing.
 */
const protocolSection = (l) => `## Self-evolving memory + skills (dsh-evolve)

You have cross-session long-term memory; relevant memories auto-inject each step
based on the user's current message (look for a "${t(l, 'anchor.recall.reference')}" block — it is context,
...
importance>=2) are injected at the START of every turn (look for a ${t(l, 'anchor.preferences.reference')}
...
/**
 * An install that predates the language setting keeps Chinese.
 *
 * English is the right default for a fresh install (issue #1), but flipping the UI of
 * somebody who has been using this in Chinese is not an upgrade, it is a surprise. The
 * evidence that an install is pre-existing has to come from the plugin's OWN state:
 * this host had no `dsh-evolve` namespace in settings.yaml at all, so "has the user
 * saved settings before" cannot answer it. MEMORY.md / a populated workspace can.
 *
 * Writes a marker so the decision is made once. Without it, a user who deliberately
 * chose 'follow-host' would be migrated back to 'zh' on every boot.
 */
function migrateLanguageForExistingInstall(cfg, workspaceDir) {
  const marker = join(workspaceDir, '.language-migrated');
  try {
    if (existsSync(marker)) return;
    // Only meaningful when the user has not expressed a choice yet.
    if (cfg.language === undefined || cfg.language === 'follow-host') {
      const looksExisting = existsSync(join(workspaceDir, 'MEMORY.md'))
        || existsSync(join(workspaceDir, 'archived-skills'))
        || existsSync(join(workspaceDir, '.git'));
      if (looksExisting) cfg.language = 'zh';
    }
    mkdirSync(workspaceDir, { recursive: true });
    writeFileSync(marker, `${new Date().toISOString()} language=${cfg.language ?? 'follow-host'}\n`);
  } catch { /* a failed migration must not stop the plugin loading */ }
}

async function _applyInner(ctx, cfg, dshHome) {
  const workspaceDir = join(dshHome, 'evolve-workspace');
  migrateLanguageForExistingInstall(cfg, workspaceDir);
      ...
      void (async () => {
        try {
          if (cfg.disposalMode === 'suggest') {
            ...
            // Fail closed when this instance does not own the workspace. Two
            // instances tidying the same store would both soft-delete from the
            // same candidate set, and the second one's generation stamps would be
            // interleaved with the first's -- so a memory could be buried by one
            // process while the other is recording that it was just used.
            // Degrading to suggest keeps the items visible without mutating.
            if (!lockVerdict.acquired) {
              suggestedDisposal = store.disposalCandidates();
              suggestedAt = Date.now();
              if (suggestedDisposal.length > 0) {
                ctx.logger?.warn?.('[dsh-evolve] automatic tidy is disabled because another '
                  + `instance owns this workspace (${lockVerdict.reason}); `
                  + `${suggestedDisposal.length} candidate(s) listed for review instead`);
              }
            } else {
  ...
  // A provider, not a string: PromptSection.text may be a function evaluated at each
  // assembly, so switching language in settings takes effect on the next turn without
  // re-registering the section (a duplicate registration throws).
  ctx.systemPrompt.section({
    name: 'evolve-protocol',
    order: 110,
    text: () => protocolSection(lang()),
  });
    ...
    async execute(args, exec) {
      ...
      if (isPending && cfg.approvalPromptEnabled === true && approvalAsk
        ...
        if (used < Number(cfg.approvalPromptMaxPerTurn ?? 1)) {
          ...
          else if (outcome === 'rejected') { await store.reject(rec.id); isPending = false; wasRejected = true; }
      ...
      // A rejection is its own outcome. Reporting it as 'confirmed' -- which is what
      // "not pending, not auto" used to collapse to -- told the model the user had
      // accepted a fact they had just declined, and the model would then rely on it.
      const status = wasRejected ? 'rejected'
        : isPending ? 'pending'
        : (isAuto ? 'auto-confirmed' : 'confirmed');
      const note = wasRejected
        ? `已按你的选择丢弃这条记忆: ${rec.kind}/imp${rec.importance} — ${rec.content.slice(0, 40)}`
        : isPending
      ...
      // saved:false on a rejection, so a caller that only checks `saved` cannot
      // conclude the memory is available.
      return { saved: !wasRejected, status, note, record: rec };
    },
  ...
  ctx.tools.register(defineTool({
    ...
    async execute() {
      ...
      if (unclaimedSkills.length) {
        suggestions.push(t(lang(), 'suggest.unclaimedSkills', { count: unclaimedSkills.length }));
      }
      if (processArtifactSuspects.length) {
        suggestions.push(t(lang(), 'suggest.processArtifacts', { count: processArtifactSuspects.length }));
      }
      if (overArchive.length) suggestions.push(t(lang(), 'suggest.archivable', { count: overArchive.length }));
      if (conv.mergeCandidates.length) suggestions.push(t(lang(), 'suggest.mergeable', { count: conv.mergeCandidates.length }));
      if (conv.bloated.length) suggestions.push(t(lang(), 'suggest.bloated', { count: conv.bloated.length }));
      if (budget.enabled && budget.overBudget) suggestions.push(t(lang(), 'suggest.overBudget', { chars: budget.overBy }));
      if (promotions.length) suggestions.push(t(lang(), 'suggest.promotable', { count: promotions.length }));
      if (fitness.ready && fitness.lowEfficiencySkills.length) {
        suggestions.push(t(lang(), 'suggest.lowEfficiency', { count: fitness.lowEfficiencySkills.length }));
      }
      return {
  ...
  ctx.on('agent/pre-step', async (payload, next) => {
    ...
    if (checkpointPending) {
      ...
      extraMessages.push(createUserMessage({
        content: [{ type: 'text', text: t(lang(), 'notice.checkpoint') }],
    ...
    if (pendingStyleOverlays.size > 0) {
      for (const [skillName, overlay] of pendingStyleOverlays) {
        extraMessages.push(createUserMessage({
          content: [{ type: 'text', text: `${t(lang(), 'notice.styleOverlay', { skill: skillName })}\n${overlay}` }],
  ...
  ctx.on('agent/error', async (payload) => {
    ...
    try {
      payload.agent.inject(createUserMessage({
        content: [{ type: 'text', text: t(lang(), 'notice.repeatedError', { count: n, fingerprint: fp }) }],
  ...
  ctx.on('session/event', (session, event) => {
    ...
    try {
      ...
      agent?.inject?.(createUserMessage({
        content: [{ type: 'text', text: `${t(lang(), 'anchor.preferences.header')}\n${snap.text}` }],
  ...
  ctx.on('session/event', async (session, event) => {
    ...
    try {
      ...
      for (const [tag, recs] of byTag) {
        ...
        lastRefineTag = tag;
        nudge(t(lang(), 'notice.skillExists', { tag, count: recs.length }));
        refined = true;
        break;
      }
      ...
      if (!refined) {
        ...
        if (ready.length > 0) {
          ...
          if (pick.tag !== lastSuggestedTag) {
            ...
            nudge(t(lang(), 'notice.skillCandidate', { tag: pick.tag, count: pick.records.length }));
          }
        }
      }
      ...
      if (overArchive.length > 0) {
        if (cfg.curatorAutoArchive === true) {
          ...
          if (done.length > 0) nudge(t(lang(), 'notice.autoArchived', { skills: done.join(', ') }));
        } else {
          nudge(t(lang(), 'notice.archiveSuggest', {
        days: cfg.curatorArchiveDays,
        skills: overArchive.map((s) => `${s.name}(${s.ageDays}d)`).join(', '),
      }));
        }
      }
      ...
      if (cfg.convergeSuggest !== false) {
        ...
        if (report.hasSuggestions) {
          ...
          if (key !== lastConvergeKey) {
            ...
            const bits = [];
            if (report.topMerge) {
              bits.push(t(lang(), 'converge.overlap', {
                a: report.topMerge.a, b: report.topMerge.b, similarity: report.topMerge.similarity,
              }));
            }
            if (report.bloated.length > 0) {
              bits.push(t(lang(), 'converge.bloated', {
                skills: report.bloated.map((x) => `${x.name}(${x.refinementCount})`).join(', '),
              }));
            }
            if (bits.length > 0) nudge(t(lang(), 'notice.convergence', { items: bits.join('; ') }));
          }
        }
      }
  ...
  try {
    ...
    const prune = makePruneController({ store, workspaceDir, cfg, skillOps, mutationsAllowed });
    ...
    setupWebSettings(ctx, {
      ...
      // A setter, not the variable: approvalAsk is declared in apply()'s scope and
      // the approval child plugin is registered from inside setupWebSettings, so
      // assigning it directly there is a cross-scope ReferenceError.
      setApprovalAsk: (fn) => { approvalAsk = fn; },
 ...
 */
function makePruneController({ store, workspaceDir, cfg, skillOps, mutationsAllowed = null }) {
  ...
  async function execute(planDigest, opts = {}) {
    // One gate for every mutating prune action, rather than one per handler: a
    // handler added later would silently miss a per-handler check, and the whole
    // point is that a second instance must not mutate this workspace at all.
    // Read from the controller's own dependency rather than from opts, because a
    // caller that forgets to pass the flag would silently disable the gate.
    // Refused before the plan is claimed, so the digest stays retryable once the
    // owning instance goes away instead of being burned as a consumed claim.
    if (mutationsAllowed && mutationsAllowed() === false) {
      return { status: 'unavailable',
        reason: 'another instance owns this workspace; pruning is disabled here' };
    }
...
function setupWebSettings(ctx, {
  ...
  proposalStore, reservations, capabilities, quarantineService,
  skillsDir, archiveDir, ownerId, idleTrigger, recordSecretIncident,
  skillTx, opRuntime, mutationsAllowed, setApprovalAsk = null,
}) {
    ...
    apply(wctx) {
      ...
      try {
        const routes = makeEvolveRoutes({
          ...
          // The settings page asks the host which language to render in, instead of
          // resolving the host locale a second time in the browser. One resolver means
          // the page and the injected prompt anchors can never disagree.
          getLanguage: () => lang(),
          getConfig: () => cfg,
          setConfig: (patch) => {
          ...
          skillOpsService: {
            listArchives: (name = null) => skillTx.listArchives(name),
            /** Frozen operations waiting for an operator decision. */
            listOperations: () => opRuntime.frozenOperations(),
            archive: async (name, opId, capClaim) => {
              const replay = opRuntime.completedReceipt(opId);
              if (replay) return { ok: true, status: 'archived', receipt: replay, replayed: true };
              if (!mutationsAllowed()) {
              ...
              const out = await skillTx.archiveSkill({ opId, source: 'web-human', name, capClaim: claim });
              return out.ok
                ? { ok: true, status: 'archived', receipt: out.receipt, archiveId: out.receipt.archiveId }
                : { ok: false, status: 'failed', reason: out.reason };
            },
            restore: async (archiveId, opId, capClaim) => {
              const replay = opRuntime.completedReceipt(opId);
              if (replay) return { ok: true, status: 'restored', receipt: replay, replayed: true };
              if (!mutationsAllowed()) {
                return { ok: false, status: 'unavailable',
                  reason: skillTx.unwritableReason ?? 'another instance owns this workspace' };
              }
              ...
              const out = await skillTx.restoreSkill({ opId, source: 'web-human', archiveId, capClaim: claim });
              return out.ok
                ? { ok: true, status: 'restored', receipt: out.receipt, name: out.receipt.targetName }
                : { ok: false, status: 'failed', reason: out.reason };
            },
            /**
             * Create a rollback PROPOSAL. Never a rollback: extracting over a live
             * directory has to be a reviewed action, and the artifact is pinned by
             * content hash here so a later backup cannot change what comes back.
             */
            proposeRollback: (name, capClaim, opId) => {
              if (!mutationsAllowed()) {
                return { ok: false, status: 'unavailable',
                  reason: skillTx.unwritableReason ?? 'another instance owns this workspace' };
              }
              const current = skillTx.readSkill(name);
              if (current === null) return { ok: false, status: 'failed', reason: `skill "${name}" not found` };
              if (capClaim?.spend) {
                const spent = capClaim.spend(opId ?? `rbp_${Date.now().toString(36)}`);
                if (!spent.ok) return { ok: false, status: 'conflict', code: spent.code, reason: spent.error };
              }
              const proposal = proposalStore.create({
                action: 'rollback', targetSkill: name, body: current,
                description: `Roll back "${name}" to its most recent pre-change backup`,
                meta: { source: 'web-human', rollback: true },
              });
              const sealed = skillTx.sealRollbackArtifact({ name, proposalId: proposal.id });
              if (!sealed.ok) {
                proposalStore.update(proposal.id, { state: 'failed', failureReason: sealed.reason });
                return { ok: false, status: 'failed', reason: sealed.reason };
              }
              proposalStore.update(proposal.id, { rollbackArtifact: sealed });
              return { ok: true, status: 'proposed', proposalId: proposal.id, artifactId: sealed.artifactId };
            },
            /**
             * An operator decides what happens to a CONFLICT/PARTIAL operation.
             *
             * The capability is spent INSIDE resolve(), because resolve() mints the
             * resolution's own opId and the claim has to name it -- spending out
             * here would bind the capability to an id that never exists.
             * `observedConflictHash` is the operator's view of the conflict: a
             * decision made against a stale view must be refused, not applied.
             */
            resolve: async (frozenOpId, decision, capClaim, observedConflictHash) => {
              if (!mutationsAllowed()) {
                return { ok: false, status: 'unavailable',
                  reason: skillTx.unwritableReason ?? 'another instance owns this workspace' };
              }
              try {
                const out = await opRuntime.resolve({
                  frozenOpId, decision, source: 'web-human',
                  // A callback, not a claim: resolve() mints the resolution's opId
                  // and the capability must be bound to it.
                  spendCapability: capClaim?.spend ? (resOpId) => capClaim.spend(resOpId) : null,
                  observedConflictHash,
                });
                return out.ok === false
                  ? { ok: false, status: 'failed', code: out.status, reason: out.error ?? 'resolution refused', detail: out.detail }
                  : { ok: true, status: 'resolved', ...out };
              } catch (e) {
    ...
    async apply(sctx) {
      try {
        ...
        const base = {
          ...
          language: ['follow-host', 'en', 'zh'].includes(cfg.language) ? cfg.language : 'follow-host',
        };
        /**
         * Resolve and apply the UI language from both layers.
         *
         * `ctx.settings.get('locale')` is the host's own namespace (owned by
         * @deepseek-ai/dsh-client-locale, persisted in $DSH_HOME/settings.yaml). It
         * returns undefined when no settings provider is mounted -- the issue reporter
         * flagged that case specifically -- so every read is guarded and falls through
         * to the resolver's default rather than throwing inside apply().
         */
        const applyLanguage = () => {
          let hostLocale;
          try { hostLocale = sctx.settings?.get?.('locale'); } catch { hostLocale = undefined; }
          setUiLanguage(resolveLanguage(cfg.language, hostLocale));
        };
        applyLanguage();

        // The host locale can change without this plugin's settings changing, so watch
        // for it. Scoped to the 'locale' namespace: every other namespace's updates are
        // other plugins' business.
        try {
          const off = sctx.on?.('settings/updated', (namespace) => {
            if (String(namespace) === 'locale') applyLanguage();
          });
          if (typeof off === 'function') sctx.effect?.(() => off);
        } catch { /* host without settings events: the override still works */ }
        ...
        installSettingsSection(sctx, ns, zEvolveSettings, base, {
          ...
          onChange: () => {
            try {
              ...
              // And the language may have just been overridden.
              applyLanguage();
            } catch { /* ignore */ }
          },
        });
  ...
  const approvalPlugin = {
    ...
    apply(actx) {
      if (typeof setApprovalAsk !== 'function') {
        // Loud, because a silent skip here is exactly how this feature spent its
        // whole life looking implemented while never running once.
        actx.logger?.warn?.('[dsh-evolve] approval prompt cannot be installed: '
          + 'no setter was provided by the host wiring');
        return;
      }
      setApprovalAsk(async ({ agent, callId, signal, reason }) => {
      ...
      actx.effect?.(() => () => { setApprovalAsk(null); });
    },
  };
  // Still guarded: the approval service is optional and its absence must never
  // break loading. But the message now names the cause, because "unavailable"
  // read identically for "host has no approval service" and "our own wiring is
  // broken" -- and it was the second one for the entire life of the feature.
