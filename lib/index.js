/**
 * @local/dsh-evolve — self-evolving memory + skill crystallization for DSH.
 *
 * v0.2.0 closes the three gaps identified against Hermes:
...
block) so they take effect immediately — the mirror of Hermes MEMORY.md. Keep these few and high-signal.
...
async function _applyInner(ctx, cfg, dshHome) {
  ...
  ctx.tools.register(defineTool({
    name: 'memory_budget',
    description: 'Show the memory char-budget status (v0.4.0 anti-bloat): total confirmed-memory chars vs the cap, and — when over budget — the lowest-value/least-reinforced/oldest trim candidates to merge or forget. Never auto-drops; this is the "over-limit returns a list" surface (Hermes-style). Read-only.',
    parameters: {},
    output: jsonOutput(),
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // tar.gz safety net before mutating the skill (mirror of Hermes curator pre-run backup).
  ...
  // ── turn/start -> Tier 1 always-on snapshot (Hermes MEMORY.md mirror) ───
 ...
 * is exactly how @local/dsh-service-admin (inject=['webServer']) works.
 */
function setupWebSettings(ctx, { cfg, store, skillStates, llm }) {
