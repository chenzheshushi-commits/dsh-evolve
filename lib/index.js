...
async function _applyInner(ctx, cfg, dshHome) {
  ...
  const mutationOrProposal = (action, payload, source = 'model-tool', hasUsageReceipt = false) => {
    ...
    return applySkillMutation({ action, source, skillsDir, archiveDir, ownerId,
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const res = mutationOrProposal('refine', { name: skillName, tag: args.tag, records, body: distilled?.body });
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const res = mutationOrProposal('fold', { name: args.name, body: distilled.body });
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (reviewState && reviewState.collector.hasContent) {
      ...
      // next review.
      const endedBadly = String(event.data?.reason?.kind ?? '') === 'error';

      const due = !endedBadly && turnNo - reviewState.lastReviewedTurn >= everyN;
      if (due) {
