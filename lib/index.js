...
async function _applyInner(ctx, cfg, dshHome) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const res = writeCrystallizedSkill(
        ...
        distilled?.body, distilled?.description, { ownerId },
      );
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const res = refineCrystallizedSkill(skillsDir, skillName, args.tag, records, ctx.logger ?? undefined, distilled?.body, { ownerId });
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const res = writeCrystallizedSkill(skillsDir, skillName, mergedTag, evidence, ctx.logger ?? undefined, body, distilled?.description, { ownerId });
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const res = foldSkillBody(skillsDir, args.name, distilled.body, ctx.logger ?? undefined, { ownerId });
  ...
  try {
    ...
    const skillOps = {
      ...
      fold: (name) => {
        ...
        return foldSkillBody(skillsDir, name, bodyBeforeRefinements(skill.body ?? ''), ctx.logger ?? undefined, { ownerId });
      },
      converge: (names, into) => {
        const skills = names.map((n) => readEvolveSkill(skillsDir, n)).filter(Boolean);
        ...
        const res = writeCrystallizedSkill(skillsDir, skillName, mergedTag, [], ctx.logger ?? undefined, body, undefined, { ownerId });
