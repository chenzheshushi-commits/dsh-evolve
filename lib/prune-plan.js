...
import {
  appendFileSync, readFileSync, existsSync, writeFileSync,
...
function redactAuditValue(value) {
  ...
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.entries(value).map(([k, v]) => [k, redactAuditValue(v)]));
  }
  return value;
}
...
export function appendAudit(workspaceDir, entry, cfg = {}, logger = { warn() {} }) {
  ...
  try {
    // Audit is itself a persistence surface. Recursively redact every string
    // before JSON serialization; otherwise a raw query/error/meta field can turn
    // the safety log into the leak. This is intentionally at the append throat so
    // present and future event types cannot forget it.
    const safe = redactAuditValue(entry);
    const line = JSON.stringify({ at: new Date().toISOString(), ...safe });
...
/**
 * Durable audit append for an automatic decision whose mutation MUST NOT happen
 * unless its intent is on disk. Unlike appendAudit this throws on every failure,
 * fsyncs the bytes, and does no ring trim (an extra O(n) failure point).
 */
export function appendAuditStrict(workspaceDir, entry, _cfg = {}) {
  const path = auditPath(workspaceDir);
  const safe = redactAuditValue(entry);
  const bytes = Buffer.from(JSON.stringify({ at: new Date().toISOString(), ...safe }) + '\n');
  let fd;
  try {
