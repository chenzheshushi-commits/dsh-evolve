import{existsSync,mkdirSync,readFileSync,writeFileSync,renameSync,openSync,closeSync,fsyncSync}from'node:fs';import{join}from'node:path';import{createHash}from'node:crypto';
...
export class EvidenceReservations{
 ...
 write(v){mkdirSync(join(this.path,'..'),{recursive:true});const tmp=`${this.path}.tmp`;const fd=openSync(tmp,'w');try{writeFileSync(fd,JSON.stringify(v,null,2)+'\n');fsyncSync(fd)}finally{closeSync(fd)}renameSync(tmp,this.path)}
