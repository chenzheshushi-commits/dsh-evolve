...
// NOT_WRITABLE must be a SUBSET of FSYNC_SOFT_FAIL, because the rethrow guard in
// flush() runs before the classification below. v0.6.3 listed EROFS here but not
// there, so EROFS threw instead of reporting not-writable -- the exact opposite of
// the "a read-only object must not abort a publish" fix it was added for, and dead
// code besides. Checked at load time rather than in a test: a member that cannot be
// classified is a contradiction in this file, not a missing assertion elsewhere.
for (const code of NOT_WRITABLE) {
  if (!FSYNC_SOFT_FAIL.includes(code)) {
    throw new Error(`lib/fsync.js: ${code} is in NOT_WRITABLE but not FSYNC_SOFT_FAIL, `
      + 'so flush() rethrows it before it can be classified');
  }
}
