 ...
 * the reference design has no "rewrite skill to user style" mechanism, but it DOES model
...
export function deriveStyleFromProfile(profile) {
  ...
  const lines = [
    '# User style overlay (applied by dsh-evolve)',
    '',
...
export function setStyle(workspaceDir, skillName, instructions, logger = { warn() {} }) {
  try {
    ...
    // Atomic because this is text the USER wrote and there is no second copy of it;
    // a crash mid-write would replace their instructions with a partial file.
