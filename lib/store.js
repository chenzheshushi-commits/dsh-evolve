 ...
 *  - Approval gate (DSWM-style): model writes land as status implied by
...
export class MemoryStore {
  ...
  stats(opts = {}) {
    ...
    const pendingQueue = all.filter((r) => r.tags.includes(PENDING_TAG))
      ...
      .map((r) => ({ id: r.id, kind: r.kind, importance: r.importance, content: String(r.content).replace(/\n/g, ' ').slice(0, 80) }));
    return {
      total: all.length,
   ...
   * Hermes MEMORY.md/USER.md. Pure filter + importance sort + char-budget
  ...
  /**
   * g3. Memory char-budget status over CONFIRMED memory (Hermes-style hard cap).
  ...
  memoryBudgetStatus() {
    ...
    return { enabled: true, ...budgetStatus(this.confirmed(), max) };
  }
  ...
  assessWrite(candidate) {
    return assessWriteQuality(candidate, this.all(), {
