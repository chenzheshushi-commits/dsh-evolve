...
export class MemoryStore {
  ...
  stats(opts = {}) {
    ...
    const topByAccess = [...all].sort((a, b) => (b.accessCount ?? 0) - (a.accessCount ?? 0)).slice(0, topN).filter((r) => (r.accessCount ?? 0) > 0).map(brief);
    const topByInjection = [...all].sort((a, b) => this.effectiveInjectionCount(b) - this.effectiveInjectionCount(a)).slice(0, topN).filter((r) => this.effectiveInjectionCount(r) > 0).map(brief);
    ...
    const pendingQueue = all.filter((r) => r.tags.includes(PENDING_TAG)).sort((a, b) => (b.importance - a.importance)).map((r) => ({ id: r.id, kind: r.kind, importance: r.importance, content: String(r.content).replace(/\n/g, ' ').slice(0, 80) }));
    return {
      total: all.length,
