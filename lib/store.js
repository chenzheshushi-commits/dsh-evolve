    ...
    for (const [id, delta] of pending) {
      ...
      try {
        await this.table.put(id, {
          ...rec,
          injectionCount: (rec.injectionCount ?? 0) + delta,
          // If tidy soft-deleted after noteInjection but before this flush, the
          // freshly used record must win and become live again. This closes all
          // three timings: during backend await, after final check, after unlock.
          forgottenAt: rec.forgottenAt ? '' : rec.forgottenAt,
        });
      } catch {
  ...
  async softForgetIfEligible(id, expected = {}, now = Date.now()) {
    ...
    try {
      await this.table.update(id, (cur) => {
        ...
        outcome = 'softDeleted';
        return { ...cur, forgottenAt: new Date(now).toISOString() };
      });
    } catch (e) {
