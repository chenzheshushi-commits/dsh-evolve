...
import { rankRecords, isExpired, tokenSetBigram, jaccard } from './search.js';
...
export class MemoryStore {
   ...
   * @param options { workspaceDir, config, logger }
   */
  ...
  async recall(query, limit, options = {}) {
    ...
    const hits = rankRecords(pool, query, limit ?? this.config.recallLimit, {
  ...
  async _evict {
    ...
    for (const v of victims) { try { await this.table.delete(v.id); } catch { /* ignore */ } }
  }
