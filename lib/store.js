...
import { buildRecallDiag } from './diag.js';
...
export class MemoryStore {
  ...
  constructor(table, options = {}) {
    this.table = table;
    // LIVE reference, not a snapshot. The host's setConfig() mutates this same
    // object in place, so a copy here would freeze every governance limit at the
    // value it had when the plugin loaded: a user lowering maxPendingQueue from 50
    // to 10 in the settings page saw the UI confirm it and /state return the new
    // value while the flood defence stayed at 50 until the process restarted.
    // Defaults are back-filled into the caller's object rather than copied out of
    // it, which keeps all 16 `this.config.*` reads honest without touching them.
    // (index.js:398 already documents this exact trap for idleTrigger, which
    // dodged it with predicates; the store did not.)
    ...
    this.onRecallDiag = typeof options.onRecallDiag === 'function' ? options.onRecallDiag : null;
    ...
    // Monotonic per-record generation. tidy snapshots the generation and rechecks
    // it inside table.update. A noteInjection after the check is protected by the
    // tombstone-aware flush below, which restores the just-used memory live.
    this._injectionGeneration = new Map();
    // Durable (bootEpoch, sequence) of the most recent in-memory signal per id.
    this._injectionStamp = new Map();
    this._generationSource = null;
  ...
  noteInjection(ids) {
    ...
    // One durable stamp per batch: it identifies WHEN this signal happened, which
    // is what lets a late-arriving use be compared against a tombstone written in
    // the meantime -- including across a restart.
    const generation = this.nextGeneration();
    for (const id of ids) {
      if (!id) continue;
      this._injectionDelta.set(id, (this._injectionDelta.get(id) ?? 0) + 1);
      this._injectionGeneration.set(id, (this._injectionGeneration.get(id) ?? 0) + 1);
      if (generation) this._injectionStamp.set(id, generation);
    }
  }
  ...
  async flushInjections() {
    ...
    // Chain-internal writes are the whole safety mechanism here. A table without
    // update() would send every flush down the retry path and persist nothing, so
    // say so once instead of degrading in silence.
    if (typeof this.table.update !== 'function') {
      this.logger.warn?.('[dsh-evolve] storage table has no update(); injection counters '
        + 'cannot be persisted safely and are being dropped');
      this._injectionDelta = new Map();
      this._injectionStamp = new Map();
      return;
    }
    const pending = this._injectionDelta;
    const stamps = this._injectionStamp;
    this._injectionDelta = new Map();
    this._injectionStamp = new Map();
    for (const [id, delta] of pending) {
      const deltaGeneration = stamps.get(id) ?? null;
      let revived = false;
      try {
        // Inside table.update, not get-then-put: the callback runs in the domain's
        // serial queue, so nothing can land between reading the record and writing
        // it back. A spread over a stale snapshot was how a tombstone written
        // during the await got silently erased -- the audit said "soft-deleted"
        // while the record was live, which is worse than either outcome alone.
        await this.table.update(id, (cur) => {
          if (!cur) return cur;
          const buried = Boolean(cur.forgottenAt);
          // A use that provably happened AFTER the tombstone is the objective
          // reason to bring the record back; an older signal must not.
          const revive = buried && this._usageBeatsTombstone(cur, deltaGeneration);
          revived = revive;
          return {
            ...cur,
            injectionCount: (cur.injectionCount ?? 0) + delta,
            ...(revive ? { forgottenAt: '', forgottenGeneration: { bootEpoch: 0, sequence: 0 } } : {}),
          };
        });
      } catch (e) {
        // A record that no longer exists is a normal outcome, not an error: the
        // delta simply has nowhere to go. Anything else is put back for retry.
        if (e?.code === 'missing-key' || /missing-key/.test(String(e?.message ?? e))) continue;
        this._injectionDelta.set(id, (this._injectionDelta.get(id) ?? 0) + delta);
        if (deltaGeneration) this._injectionStamp.set(id, deltaGeneration);
        continue;
      }
      if (revived) {
        try { this.fts?.upsert(this.table.get(id)); } catch { /* best-effort */ }
        this.logger.warn?.('[dsh-evolve] a memory was used again after being tidied away; '
          + 'it has been restored (id ' + id + ')');
      }
    }
  }
  ...
  /**
   * Where durable generation stamps come from (the boot epoch).
   *
   * Injected rather than constructed here because the epoch may be UNAVAILABLE:
   * if it cannot be established safely the source returns null, and tidy must
   * then refuse to write tombstones rather than stamp them with a number it made
   * up. A wrong generation is worse than none -- it silently converts a real new
   * use into a stale signal.
   */
  setGenerationSource(fn) { this._generationSource = typeof fn === 'function' ? fn : null; }

  /** Next durable generation stamp, or null when the epoch is unusable. */
  nextGeneration() {
    try { return this._generationSource?.() ?? null; } catch { return null; }
  }

  /**
   * Is this in-memory usage signal newer than the tombstone's stamp?
   *
   * Compared as (bootEpoch, sequence): after a restart the sequence starts over,
   * so a raw counter comparison would judge the very first genuine use of the new
   * run to be older than anything from the previous one.
   */
  _usageBeatsTombstone(record, deltaGeneration) {
    const t = record?.forgottenGeneration;
    const tEpoch = Number(t?.bootEpoch ?? 0);
    const tSeq = Number(t?.sequence ?? 0);
    // An unstamped tombstone cannot be ordered against anything: it predates
    // v0.6.0, or was written while the boot epoch was unusable. Since we cannot
    // prove the use is older, we restore -- burying a memory the user just used is
    // the worse error, and it is the invisible one.
    if (tEpoch === 0 && tSeq === 0) return true;
    const dEpoch = Number(deltaGeneration?.bootEpoch ?? 0);
    const dSeq = Number(deltaGeneration?.sequence ?? 0);
    // An unstamped signal against a stamped tombstone is the reverse case: the
    // tombstone is known-recent and the signal's age is unknown, so it loses.
    if (dEpoch === 0 && dSeq === 0) return false;
    if (dEpoch !== tEpoch) return dEpoch > tEpoch;
    return dSeq > tSeq;
  }
  ...
  async recall(query, limit, options = {}) {
    ...
    if (this.fts?.available) {
      try {
        ...
        if (ftsIds.length > 0) {
          // Same scoring options rankRecords used above: the fusion re-checks each FTS
          // hit against the scorer, and a different recency setting there would apply a
          // different threshold than the one that produced bigramHits.
          hits = fuseRRF(bigramHits, ftsIds, this.recordsById(), want, query, {
            recencyHalfLifeDays: this.config.recencyHalfLifeDays,
          });
        }
      } catch {
    ...
    if (options.touch) {
      // Chain-internal update, never a spread over `record`. `record` is the
      // snapshot recall saw; writing it back wholesale would undo anything that
      // landed in between -- including a tombstone -- leaving the audit claiming a
      // soft delete that silently did not stick. A tombstoned record is skipped
      // rather than resurrected: a manual recall is a read, and reviving on read
      // would make tidy undoable by merely searching.
      for (const { record } of hits) {
        try {
          await this.table.update(record.id, (cur) => {
            if (!cur || cur.forgottenAt) return cur;
            return { ...cur, accessedAt: nowIso(), accessCount: (cur.accessCount ?? 0) + 1 };
          });
        } catch (e) {
          if (e?.code === 'missing-key' || /missing-key/.test(String(e?.message ?? e))) continue;
          // Not swallowed: a failing access stamp is a real signal, and the old
          // empty catch is why this bug stayed invisible.
          this.logger.warn?.(`[dsh-evolve] recall touch failed for ${record.id}: ${e?.message ?? e}`);
        }
      }
    }
    try {
      this.onRecallDiag?.(buildRecallDiag(query, {
        hits: hits.length,
        ms: Number((performance.now() - diagStarted).toFixed(3)),
        path,
        source: options.source ?? (options.touch ? 'manual' : 'injection'),
      }));
    } catch { /* diagnostics are observational and never block recall */ }
    return hits;
  }
  ...
  async softForget(id, confirmSoft) {
    ...
    // The tombstone records WHEN it was written, so a usage signal that arrives
    // afterwards can be recognised as newer and bring the record back.
    const generation = this.nextGeneration() ?? { bootEpoch: 0, sequence: 0 };
    await this.table.update(id, (cur) => (cur
      ? { ...cur, forgottenAt: nowIso(), forgottenGeneration: generation } : cur));
    await this._afterWrite();
  ...
  async restoreForgotten(id) {
    ...
    await this.table.update(id, (cur) => {
      if (!cur) return cur;
      updated = { ...cur, forgottenAt: '', forgottenGeneration: { bootEpoch: 0, sequence: 0 } };
      return updated;
    });
    if (!updated) return { restored: 0 };
    await this._afterWrite();
   ...
   */
  /** Same eligibility predicate used by suggest and tidy at execution time. */
  isDisposalEligible(r, now = Date.now()) {
    if (!r || r.forgottenAt || r.pinned) return false;
    if (new Set(['preference', 'decision']).has(r.kind)) return false;
    if (r.tags?.includes(PENDING_TAG) || isRejected(r) || r.importance === 3) return false;
    if (this.effectiveInjectionCount(r) !== 0 || (r.accessCount ?? 0) !== 0) return false;
    const minIdleMs = Math.max(0, Number(this.config.disposalMinIdleDays ?? 30)) * 86400000;
    return now - Date.parse(r.createdAt || 0) >= minIdleMs;
  }

  /**
   * CAS-like tidy soft delete: the predicate and usage generations are checked
   * INSIDE table.update's serialized callback against the latest record.
   */
  async softForgetIfEligible(id, expected = {}, now = Date.now()) {
    let outcome = 'skipped';
    // Stamped before the update so the tombstone's generation is strictly older
    // than any usage signal produced after this point. Without a usable epoch the
    // stamp is zeroed, which the flush path treats as "older than any use" --
    // fail-safe towards restoring rather than burying.
    const generation = this.nextGeneration() ?? { bootEpoch: 0, sequence: 0 };
    try {
      await this.table.update(id, (cur) => {
        if (!this.isDisposalEligible(cur, now)) { outcome = 'no-longer-eligible'; return cur; }
        if ((this._injectionGeneration.get(id) ?? 0) !== (expected.injectionGeneration ?? 0)) {
          outcome = 'used-since'; return cur;
        }
        if (this.effectiveInjectionCount(cur) !== (expected.injectionCount ?? 0)
          || (cur.accessCount ?? 0) !== (expected.accessCount ?? 0)) {
          outcome = 'used-since'; return cur;
        }
        outcome = 'softDeleted';
        return {
          ...
          forgottenGeneration: generation,
        };
      });
    } catch (e) {
      if (e?.code === 'missing-key' || /missing/i.test(String(e?.message ?? e))) outcome = 'gone';
      else throw e;
    }
    ...
    return { softDeleted: outcome === 'softDeleted' ? 1 : 0, skipped: outcome };
  }

  disposalCandidates(now = Date.now()) {
    const out = [];
    for (const r of this.all()) {
      if (!this.isDisposalEligible(r, now)) continue;
      ...
      out.push({
        ...
        accessCount: r.accessCount ?? 0,
        injectionCount: this.effectiveInjectionCount(r),
        injectionGeneration: this._injectionGeneration.get(r.id) ?? 0,
  ...
  memoryBudgetStatus() {
    ...
    // Feed EFFECTIVE injection counts: budgetStatus uses injectionCount as a
    // tie-break, and the persisted field lags behind by any not-yet-flushed
    // in-memory delta. Two definitions of "how used is this record" in one codebase
