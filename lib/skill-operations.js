...
import { writeFileAtomic, isTempName } from './atomic-write.js';
    ...
    return this.#runLocked({
      ...
      manifest: () => {
        ...
        const common = {
          ...
          expectedAfterContentHash: afterHashes.contentHash,
          expectedAfterStateHash: afterHashes.stateHash,
          // A PREFIX, not an exact path: the real staging file carries a random suffix
          // (see lib/atomic-write.js). Recorded unchanged so the op-manifest schema and
          // its conformance corpus keep their existing values.
          tmpPath: `${file}.tmp`,
          finalPath: file,
          backupPath: 'pending',
          marker,
    ...
    for (const entry of report.pending) {
      ...
      try {
        if (entry.verdict === 'roll-forward') {
          ...
          // Protocol B's staging file now has a RANDOM suffix, so the exact path cannot be
      // known after a crash. `tmpPath` is therefore a PREFIX (`<final>.tmp` is what the
      // manifest still records), and cleanup sweeps the directory for leftovers whose
      // name starts with the final file's name and ends with the temp suffix. Sweeping
      // by suffix is a pattern this module's sibling already uses -- skill-proposals.js
      // lists its root and filters `.tmp`.
      if (m.protocol === 'B' && m.finalPath) sweepTempFiles(m.finalPath);
 ...
 *
 * Delegates to lib/atomic-write.js. This was one of two functions called
 * `writeFileDurable` with DIFFERENT semantics -- the one in publish-protocols.js
 * truncated the destination in place -- which is exactly the confusion a single
 * implementation removes.
 */
/**
 * Remove any leftover atomic-write temp files for `finalPath`.
 *
 * Random temp names mean a crashed write cannot be cleaned up by remembering one
 * path, so recovery sweeps instead: same directory, same base name, temp suffix.
 */
function sweepTempFiles(finalPath) {
  const dir = dirname(finalPath);
  if (!existsSync(dir)) return;
  const base = `${basename(finalPath)}.`;
  for (const entry of readdirSync(dir)) {
    if (!entry.startsWith(base) || !isTempName(entry)) continue;
    try { rmSync(join(dir, entry), { force: true }); } catch { /* recovery is best effort */ }
  }
}
