...
export function publishSkillFile({ file, body, expect = null, opId, logger = { warn() {} } }) {
  ...
  // Protocol B's commit point. This was a hand-rolled temp+rename on the fixed name
  // `${file}.tmp`, which is the shape that let a pre-planted symlink at that exact
  // path redirect the write outside the skill tree (measured), and let a second
  // writer truncate the first one's bytes before any fsync could help.
