...
export function getOwnerId(workspaceDir) {
  ...
  // Atomic because ownerId is random and unrecoverable: a truncated file here means
  // this workspace silently becomes a different owner, and every skill it had claimed
  // reads as belonging to someone else.
  writeFileAtomic(path, `${JSON.stringify({ ownerId, createdAt: new Date().toISOString() }, null, 2)}\n`, 0o600);
  return ownerId;
}
