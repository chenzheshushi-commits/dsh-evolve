...
import { memoryHeat } from './heat.js';
import { PROTECTED_KINDS } from './prune-authz.js';

/** Local heat resolver: use precomputed _heat if present, else compute. */
function heatFor(r, cfg) {
  return (typeof r._heat === 'number') ? r._heat : memoryHeat(r, cfg);
}
 ...
 */
/**
 * `records[].injectionCount` must already be the EFFECTIVE count (persisted value
 * plus any unflushed in-memory delta). This module cannot compute it -- only the
 * store knows about unflushed deltas -- so store.memoryBudgetStatus() substitutes
 * it before calling. Reading the raw field here would give a second, staler
 * definition of the same signal.
 */
export function budgetStatus(records, maxChars, cfg = {}) {
  ...
  let trimCandidates = [];
  if (overBudget) {
    // v0.4.2: pinned + protected-kind records are NEVER prune candidates (they
    // don't just sort last — they're excluded). heat is the 4th sort key, placed
    // BEFORE the timestamp tie-break so it actually takes effect (a 5th key after
    // a string timestamp would almost never fire). heat is time-coldness only;
    // importance/observationCount/injectionCount rank ahead of it as value signals.
    const eligible = records.filter((r) => !r.pinned && !PROTECTED_KINDS.includes(r.kind));
    trimCandidates = eligible
      .slice()
      .sort((a, b) => (
        (a.importance - b.importance)
        || ((a.observationCount ?? 1) - (b.observationCount ?? 1))
        || ((a.injectionCount ?? 0) - (b.injectionCount ?? 0))
        || ((a._heat ?? heatFor(a, cfg)) - (b._heat ?? heatFor(b, cfg)))
      ...
      .map((r) => ({
        ...
        heat: Number((r._heat ?? heatFor(r, cfg)).toFixed(3)),
