 ...
 * group, rewrite as steps + pitfalls) — raising skill write-up quality.
 *
 * EFFICIENCY (auxiliary task budget):
...
export async function refineWithLLM(ctx, exec, { rawText, kind, tag, cfg, logger }) {
  ...
  try {
    ...
    const messages = [createUserMessage({
      ...
      source: { kind: 'plugin', plugin: 'dsh-evolve', form: 'notice', summary: 'skill 精炼取证' },
    })];
