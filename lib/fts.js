...
/**
 * Space-joined encoded tokens for indexing a record. R3 (v0.5.0): index BOTH the
 * content AND the tags — tags are a human/model-curated alias layer (e.g. a
 * record about "中文回复" tagged 语言/中文), so folding them into the index bridges
 * part of the synonym gap that pure content bigrams miss, at zero extra dependency
 * (cjkmem proved `blob = content + " " + tags` works). Tags are joined with spaces
 * so their bigrams don't bleed across tag boundaries.
 */
function encodeText(textOrRecord) {
  // Back-compat: a bare string still works (content-only). A record object also
  // contributes its tags.
  if (typeof textOrRecord === 'string') return tokenizeBigram(textOrRecord).map(encodeToken).join(' ');
  const rec = textOrRecord ?? {};
  const content = String(rec.content ?? '');
  const tags = Array.isArray(rec.tags) ? rec.tags.filter((t) => t !== 'pending' && t !== 'auto-confirmed').join(' ') : '';
  const blob = tags ? `${content} ${tags}` : content;
  return tokenizeBigram(blob).map(encodeToken).join(' ');
}
...
class FtsIndex {
  ...
  upsert(record) {
    ...
    try {
      ...
      this._insert.run(record.id, encodeText(record));  // R3: content + tags
