 ...
 *
 * The comment said "atomically" while the body opened a FIXED `${final}.tmp` with
 * mode 'w' -- the third copy of that shape in the repo, under a third name, which is
 * why a gate keyed on function names would not have found it. The mechanism now
 * lives in lib/atomic-write.js; this function keeps only the JSON framing.
 */
function writeJsonAtomic(dir, name, value) {
  mkdirSync(dir, { recursive: true });
  const final = join(dir, name);
  // 0o644: the op registry is state an operator is expected to be able to read.
  writeFileAtomic(final, `${JSON.stringify(value, null, 2)}\n`, 0o644);
}
...
/**
 * Frozen phases are terminal for the operation itself, but an operator
 * resolution is allowed to move them exactly once -- that is what stops a
 * CONFLICT from holding its capability and receipts forever. Every other
 * terminal really is final.
 */
const RESOLVABLE = new Set(['CONFLICT', 'PARTIAL']);
const RESOLVED_ONLY = new Set([
...
export class OpRegistry {
  ...
  advance(opId, patch) {
    ...
    if (TERMINALS.has(current.phase) && patch.phase && patch.phase !== current.phase) {
      // The single legal exception: an operator resolution moving CONFLICT/PARTIAL
      // into its matching resolved terminal. Without it the frozen operation's
      // capability and receipts could never be released (plan 0R.2).
      const resolving = RESOLVABLE.has(current.phase) && RESOLVED_ONLY.has(patch.phase);
      if (!resolving) {
