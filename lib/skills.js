 ...
 *     back) and never destroys — archive-not-delete is the rule
...
import {
  ...
  renameSync, rmSync, openSync, closeSync, fsyncSync,
} from 'node:fs';
import { join, dirname } from 'node:path';
import { execFileSync } from 'node:child_process';
import { fsyncDir } from './fsync.js';
import { writeFileAtomic as writeFileAtomicShared } from './atomic-write.js';

const NAME_RE = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
/** Frontmatter marker proving a skill is evolve-owned (safe to touch/refine/archive). */
// Ownership is decided in skill-ownership.js, structurally. Keep the marker here
// for rendering only -- deciding ownership from a substring of the prose was the
// authorization hole this module exists to close.
import {
  classifyOwnership, isEvolveOwned as ownershipCheck, readAuthor as readAuthorLine,
  claimState, OWNERSHIP,
...
/**
 * Same-directory temp+fsync+rename: readers see old or complete new bytes.
 *
 * The four lines that used to live here (fixed `${file}.tmp` opened with 'w') are now
 * one shared implementation in lib/atomic-write.js. Seven modules had their own copy
 * of them, and the fixed name is what made symlink pre-placement and concurrent
 * truncation possible -- see that file for the measured reproductions.
 */
function writeFileAtomic(file, body) {
  // 0o644: SKILL.md is content the agent and the user both read.
  writeFileAtomicShared(file, body, 0o644);
}
...
export function findSkillByTag(skillsDir, tag) {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      // Structural, not substring: prose mentioning the marker is not ownership.
      if (readAuthorLine(md) !== EVOLVE_MARKER) continue;
 ...
 * of the tag-template. */
export function renderSkillMd(name, tag, records, distilledBody, llmDescription, ownerId = null) {
  const version = '1.0.0';
  ...
  const state = {
    ...
    version,
    createdAt: nowIso(),
    // Binds this skill to THIS installation. Without it the author line and the
    // state block are just public text anyone could copy.
    ...(ownerId ? { ownerId } : {}),
...
/**
 * Is this SKILL.md safe for us to rewrite?
 *
 * Structural check, not a substring one: a hand-written skill that merely
 * mentions the marker in its prose is NOT ours. Pass ownerId to enforce the
 * installation binding; allowLegacy lets manual tools touch pre-ownerId skills
 * that autonomous paths must leave alone.
 */
function isEvolveOwned(file, ownerId = null, opts = {}) {
  try {
    return ownershipCheck(readFileSync(file, 'utf8'), ownerId, opts);
  } catch { return false; }
}
 ...
 */
export function writeCrystallizedSkill(skillsDir, name, tag, records, logger = { warn() {} }, distilledBody, llmDescription, opts = {}) {
  const ownerId = opts.ownerId ?? null;
  try {
    const dir = join(skillsDir, name);
    const file = join(dir, 'SKILL.md');
    // Overwriting an existing skill: it must be ours. allowLegacy because a
    // pre-ownerId skill we created is still ours -- refusing would make our own
    // older output un-updatable.
    if (existsSync(file) && !isEvolveOwned(file, ownerId, { allowLegacy: true })) {
      const { status, reason } = classifyOwnership(readFileSync(file, 'utf8'), ownerId);
      logger.warn?.(`skill ${name} exists and is not evolve-owned (${status}: ${reason}); skipping`);
      return null;
    }
    mkdirSync(dir, { recursive: true });
    writeFileAtomic(file, renderSkillMd(name, tag, records, distilledBody, llmDescription, ownerId));
 ...
 */
export function refineCrystallizedSkill(skillsDir, name, tag, records, logger = { warn() {} }, distilledBody, opts = {}) {
  try {
    const planned = renderRefinedSkill(skillsDir, name, tag, records, distilledBody, { logger, ...opts });
    if (!planned || planned.refined === false) return planned;
    writeFileAtomic(planned.path, planned.md);
    _touchUsage(skillsDir, name, `refined:v${planned.version}`);
    return { name, path: planned.path, version: planned.version, added: planned.added, refined: true };
  } catch (e) {
...
/**
 * Everything refine does EXCEPT touching the disk: returns the exact bytes that
 * would be published.
 *
 * The durable operation protocol needs this split. It has to hash the result and
 * record an intent BEFORE the rename, and it stamps the committing operation id
 * into the state block so prose and proof land in the same rename. A function
 * that renders and writes in one step cannot participate in that.
 */
export function renderRefinedSkill(skillsDir, name, tag, records, distilledBody, opts = {}) {
  const ownerId = opts.ownerId ?? null;
  const logger = opts.logger ?? { warn() {} };
  {
    ...
    const md = readFileSync(file, 'utf8');
    // allowLegacy: this is a manual tool, so a pre-ownerId skill may still be
    // refined by hand. Autonomous paths pass an ownerId and get the strict answer.
    if (!ownershipCheck(md, ownerId, { allowLegacy: true })) {
      const { status, reason } = classifyOwnership(md, ownerId);
      logger.warn?.(`skill ${name} is not evolve-owned (${status}); refusing to refine`);
      return { refined: false, reason: `skill "${name}" is not evolve-owned: ${reason}` };
    }
    ...
    return { name, path: file, version: newVersion, added: fresh.length, md: out, state: nextState };
  }
}
 ...
 * | null. Never throws.
 */
export function foldSkillBody(skillsDir, name, foldedBody, logger = { warn() {} }, opts = {}) {
  try {
    const planned = renderFoldedSkill(skillsDir, name, foldedBody, { logger, ...opts });
    if (!planned || planned.folded === false) return planned;
    writeFileAtomic(planned.path, planned.md);
    _touchUsage(skillsDir, name, `folded:v${planned.version}`);
    return { name, path: planned.path, version: planned.version, folded: true };
  } catch (e) {
...
/** Fold's rendered bytes without publishing them (see renderRefinedSkill). */
export function renderFoldedSkill(skillsDir, name, foldedBody, opts = {}) {
  const ownerId = opts.ownerId ?? null;
  const logger = opts.logger ?? { warn() {} };
  {
    const dir = join(skillsDir, name);
    ...
    if (!ownershipCheck(md, ownerId, { allowLegacy: true })) {
      const { status, reason } = classifyOwnership(md, ownerId);
      return { folded: false, reason: `skill "${name}" is not evolve-owned (${status}): ${reason}` };
    }
    ...
    const provenance = [
      '',
      `> Auto-crystallized by dsh-evolve (folded v${newVersion}).`,
      '> Refinement sections were distilled back into the body below. dsh-evolve',
      '',
    ].join('\n');
    ...
    const out = `${body.replace(/\s+$/, '')}\n\n${renderState(nextState)}\n`;
    return { name, path: file, version: newVersion, md: out, state: nextState };
  }
}
...
function moveDir(src, dst) {
  ...
  try {
    ...
    if (e?.code === 'EXDEV') {
      throw new Error('cross-filesystem skill move refused: archive/restore must be an atomic rename, never copy+delete');
    }
    throw e;
  }
}
 ...
 */
export function archiveSkill(skillsDir, archiveDir, name, logger = { warn() {} }, opts = {}) {
  const ownerId = opts.ownerId ?? null;
  try {
    ...
    // Archive is a reversible move that changes no content, and it is a manual
    // tool, so a legacy skill may still be archived by hand.
    if (!isEvolveOwned(file, ownerId, { allowLegacy: true })) {
      const { status, reason } = classifyOwnership(readFileSync(file, 'utf8'), ownerId);
      return {
        archived: false,
        reason: `skill "${name}" is not evolve-owned (${status}): ${reason}`,
      };
    }
    const dst = join(archiveDir, name);
    if (existsSync(dst)) return { archived: false, reason: `archive target already exists for "${name}"; refusing to overwrite user asset` };
    mkdirSync(archiveDir, { recursive: true });
    // Archive is a reversible move that changes no content, so a tar backup is a
    // convenience, not a precondition -- the reverse move restores it. Failure is
    // logged rather than swallowed, so a broken tar does not stay invisible.
    try {
      backupSkill(skillsDir, archiveDir, name, 'archive');
    } catch (e) {
      logger.warn?.(`archive backup for ${name} failed (${e?.message ?? e}); `
        + 'continuing because archive is reversible by moving it back');
    }
    moveDir(src, dst);
 ...
 * taken before a destructive op (archive/refine). The point is that a snapshot is
 ...
 */
/**
 * Take a tar.gz snapshot of a skill before mutating it.
 *
 * Throws on failure. Callers that REWRITE content (create/refine/fold/converge/
 * rollback) must let it throw: continuing without a backup means an LLM rewrite
 * or an rmSync has no way back. If tar is unavailable those operations are simply
 * unavailable, and saying so is better than silently destroying the only copy.
 *
 * archive/restore/autoArchive do NOT need this: they are reversible moves that
 * change no content, so the reverse move is the recovery path.
 */
...
function readMeta(file) {
  try {
    ...
    const st = readState(md) ?? {};
    return {
      owned: readAuthorLine(md) === EVOLVE_MARKER,
      ownership: classifyOwnership(md, null).status,
 ...
 */
/**
 * Skills that look like ours but predate the ownerId binding.
 *
 * Report only, and deliberately so: automatically stamping every file that looks
 * like ours would be claiming the user's own work. Until a human claims them they
 * stay out of every autonomous path, though manual tools still work on them.
 */
export function listUnclaimedSkills(skillsDir, ownerId) {
  const out = [];
  try {
    for (const entry of readdirSync(skillsDir, { withFileTypes: true })) {
      ...
      try {
        const { status, state } = classifyOwnership(readFileSync(file, 'utf8'), ownerId);
        if (status !== OWNERSHIP.LEGACY_UNCLAIMED) continue;
        out.push({
          name: entry.name,
          tag: state?.tag ?? null,
      ...
      } catch { /* unreadable: not a claim candidate */ }
    }
...
/**
 * Stamp this installation's ownerId into a legacy skill. Explicit action only.
 *
 * Refuses anything that is not LEGACY_UNCLAIMED, so this cannot be used to take
 * over a foreign skill or to re-stamp one that already belongs elsewhere.
 */
export function claimLegacySkill(skillsDir, name, ownerId, logger = { warn() {} }) {
  try {
    if (!NAME_RE.test(name)) return { claimed: false, reason: `invalid skill name "${name}"` };
    if (!ownerId) return { claimed: false, reason: 'no ownerId for this installation' };
    const file = join(skillsDir, name, 'SKILL.md');
    if (!existsSync(file)) return { claimed: false, reason: `skill "${name}" not found` };
    const md = readFileSync(file, 'utf8');
    const { status, state, reason } = classifyOwnership(md, ownerId);
    if (status !== OWNERSHIP.LEGACY_UNCLAIMED) {
      return { claimed: false, reason: `skill "${name}" is ${status}: ${reason}` };
    }
    const next = claimState(state, ownerId);
    const updated = md.replace(STATE_RE, renderState(next));
    if (updated === md) return { claimed: false, reason: 'could not rewrite the state block' };
    writeFileAtomic(file, updated);
    return { claimed: true, name, ownerId };
  } catch (e) {
    logger.warn?.(`claimLegacySkill failed: ${e?.message ?? e}`);
    return { claimed: false, reason: String(e?.message ?? e) };
  }
}
...
export function countCrystallizedSkills(skillsDir) {
  try {
    ...
    for (const entry of readdirSync(skillsDir, { withFileTypes: true })) {
      ...
      try {
        if (statSync(f).isFile() && readAuthorLine(readFileSync(f, 'utf8')) === EVOLVE_MARKER) n += 1;
      } catch { /* skip */ }
    }
    return n;
  } catch { return 0; }
