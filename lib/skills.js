...
function writeFileAtomic(file, body) {
  ...
  renameSync(tmp, file);
  // Shared helper, not a private copy: this line used to open the directory itself
  // and swallow EVERY error, including a real ENOSPC/EIO. It was also the one
  // path-level fsync the v0.6.2 gate could not see -- its regex could not cross the
  // nested call in `dirname(file)`, so the module read as compliant.
