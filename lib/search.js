...
export function matchBaseMin(query) {
  ...
  //
  // ⚠ CORRECTION (v0.6.2). This comment used to claim that unrelated records
  // "score exactly 0 — a wide safety gap", and that the 0.6 floor "still rejects
  // lone-fragment coincidence". BOTH ARE FALSE, measured against this scorer:
  //
  //   编程语言 vs '用户偏好：回复语言必须是中文'          -> 1.41 (floor 0.80)
  //   13-char query vs a record sharing only 存储/处理 -> 1.27-3.07 (floor 0.60)
  //
  // A lone 2-gram earns FRAGMENT (0.6) below, which equals this floor on its own,
  // so any Jaccard credit at all carries it over. The gap does not exist. It cannot
  // be closed by raising the floor either: false positives (1.41, 3.07) straddle a
  // real paraphrase hit (1.47). The fix has to change the fragment rule and needs
  // baseline/retrieval-baseline.mjs recall+MRR evidence -- see
  // scripts/schema/f3-precision-hole.test.mjs for the numbers and the ratchet.
