...
export function matchBaseMin(query) {
  ...
  //
  // A lone 2-gram earns FRAGMENT (0.6) below, so a single shared word reaches this
  // floor unaided and any Jaccard credit carries it over. Raising the floor cannot
  // fix it: the unrelated report (1.7541) outscores the real hit (0.8393) by more
  // than 2x, so no threshold admits one and rejects the other. Only the fragment
  // rule can change that, and it needs baseline/retrieval-baseline.mjs recall+MRR
  // evidence -- see scripts/schema/f3-precision-hole.test.mjs and issue #2.
