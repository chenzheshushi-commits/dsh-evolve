 ...
 *
 * Returns { ok, unsynced, unsupported }:
 *
 ...
 *   ok           false only when a real I/O error is possible, i.e. never here --
 *                genuine errors (ENOSPC, EIO) are thrown by fsync.js, not returned.
 *
...
export function fsyncTree(root) {
  const unsynced = [];
  const unsupported = [];
  if (!existsSync(root)) return { ok: true, unsynced, unsupported };
  const dirs = [];
  const walk = (dir) => {
    dirs.push(dir);
