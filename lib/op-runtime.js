...
const FSYNC_SOFT_FAIL = ['EINVAL', 'EACCES', 'EPERM', 'EISDIR', 'ENOTSUP'];

function fsyncPath(path, mode) {
  let fd;
  try {
    fd = openSync(path, mode);
    fsyncSync(fd);
  } catch (e) {
    // Some filesystems refuse fsync on a directory opened O_RDONLY, and Windows
    // refuses it on any handle without write access. Losing the durability of a
    // rename is not fatal (the data is already written), but a real error must
    // not be hidden.
    if (!FSYNC_SOFT_FAIL.includes(e?.code)) throw e;
  } finally {
    if (fd !== undefined) try { closeSync(fd); } catch { /* gone */ }
  }
}

export function fsyncDir(dir) { fsyncPath(dir, 'r'); }

/**
 * fsync a regular file. Must open 'r+', not 'r': Windows FlushFileBuffers needs
 * GENERIC_WRITE, so an O_RDONLY handle fails with EPERM and the file would be
 * left un-synced -- silently, because that EPERM is soft-failed above. 'w' would
 * truncate. fsyncTree's whole contract (a marker written after it proves the tree
 * is durable) depends on the files actually reaching disk.
 */
export function fsyncFile(file) { fsyncPath(file, 'r+'); }

/**
 * fsync an entire tree: every regular file first, then every directory from the
 * leaves up. A marker written afterwards therefore proves the whole tree is
 * durable, which is the property protocol C/D recovery depends on (plan 0K.6).
 */
  ...
  reconcile({ skillsDir = null, archiveDir = null } = {}) {
    ...
    if (existsSync(opsRoot)) {
      for (const opId of readdirSync(opsRoot)) {
        ...
        report.pending.push({ opId, phase: m.phase, kind: m.kind, verdict: verdict.verdict, reason: verdict.reason });
  ...
  classify(manifest, { skillsDir = null, archiveDir = null } = {}) {
    ...
    if (manifest.kind === 'move') {
      ...
      if (destExists) {
        ...
        if (marker?.opId === opId) return { verdict: 'roll-forward', reason: 'destination carries this operation marker' };
        return {
          verdict: 'conflict',
    ...
    if (manifest.protocol === 'A' || manifest.protocol === 'C') {
      ...
      if (final && existsSync(final)) {
        ...
        if (marker?.opId === opId) return { verdict: 'roll-forward', reason: 'published directory carries this operation marker' };
    ...
    if (manifest.kind === 'converge') {
      ...
      if (target.finalPath && existsSync(target.finalPath)) {
        ...
        if (marker?.opId === opId) {
          return { verdict: 'roll-forward', reason: 'the umbrella carries this operation marker; finish archiving' };
        }
