 ...
 *
 * Returns { ok, unsynced }. `ok` is false when a regular FILE could not be
 * flushed, and the caller must not then publish a marker: the marker's entire
 * meaning is "these bytes are on disk". Directory refusals do not clear `ok` --
 * they only cost the durability of a rename, whose data is already written.
 *
 * v0.6.0 had no return value at all and swallowed file refusals one layer down,
 * so on Windows this function flushed NOTHING while every caller went on to
 * publish a marker claiming otherwise.
 */
export function fsyncTree(root) {
  if (!existsSync(root)) return { ok: true, unsynced: [] };
  const dirs = [];
  const unsynced = [];
  const walk = (dir) => {
    dirs.push(dir);
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (entry.isFile() && !fsyncFile(full)) unsynced.push(full);
    }
  };
  walk(root);
  for (const dir of dirs.reverse()) fsyncDir(dir);
  return { ok: unsynced.length === 0, unsynced };
}
