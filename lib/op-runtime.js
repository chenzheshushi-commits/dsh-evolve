...
function fsyncPath(path) {
  ...
  try {
    ...
    // Some filesystems refuse fsync on a directory opened O_RDONLY. Losing the
    // durability of a rename is not fatal (the data is already written), but a
    // real error must not be hidden.
...
export function fsyncDir(dir) { fsyncPath(dir); }
...
export function fsyncTree(root) {
  ...
  const walk = (dir) => {
    ...
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (entry.isFile()) fsyncPath(full);
    }
  };
  walk(root);
...
export class InstanceLock {
  ...
  ownerIsDead(owner) {
    ...
    const expired = this.now() - Number(owner.acquiredAtMs ?? 0) > this.ttlMs;
    if (!expired) return false;
    const pid = Number(owner.pid ?? 0);
    if (!pid || !pidAlive(pid)) return true;
    const token = processStartToken(pid);
    return Boolean(owner.processStartToken && token && owner.processStartToken !== token);
  }
...
export class OperationRuntime {
   ...
   */
  async resolve({ frozenOpId, decision, capClaim = null, source = 'web-user', observedConflictHash }) {
    ...
    const resolution = {
      ...
      capClaim: isObj(capClaim) ? { ...capClaim, claimedByOpId: resolutionOpId } : capClaim,
