 ...
 *
 * This used to be `openSync(file, 'w')` -- the one thing lib/fsync.js bans in plain
 * text ("openSync(file, 'w') + fsyncSync -> OK, but TRUNCATES -- never use it here"),
 * with no gate enforcing the ban. It mattered because publishMove writes its marker
 * into an EXISTING live skill directory, and when its cleanup fails the marker is
 * deliberately left behind for recovery; the next retry then reopened that live file
 * with 'w'. Measured consequence of crashing inside that window: a 65-byte marker
 * became 0 bytes, readMarker returned {corrupt:true}, and the operation could no
 * longer be attributed -- so it could never roll forward, only freeze.
 *
 * Temp+fsync+rename gives the property the rest of the repo already promises:
 * a reader sees the old marker or the complete new one, never a truncated one.
 */
export function writeMarker(dir, marker) {
  const file = join(dir, MARKER_NAME);
  // 0o644: recovery tooling and operators both read markers.
...
export class BootEpoch {
  ...
  #write(epoch) {
    ...
    // Was a fixed `${this.path}.tmp` opened with 'w'. The boot epoch decides whether
    // generations are available at all, so a truncated one fails the whole plugin
    // closed.
    writeFileAtomic(this.path, body, 0o644);
  }
