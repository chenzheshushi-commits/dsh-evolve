...
export const MemoryRecordSchema = z.object({
  ...
  // v0.4.2 soft-delete tombstone: non-empty ISO string = forgotten (recoverable,
  // never physically deleted by softForget). Excluded from every active-record
  // consumer via confirmed(); read/write/evict paths all honor it. Empty = live.
  forgottenAt: z.string().default(''),
  // v0.4.2 user pin: true = user explicitly protected this record. Never enters
  // prune candidates, never reinforced/overwritten, needs confirm to delete.
  // MUST have .default(false) — a new record field without default bricks open()
  // on existing stores (schemastery valueSchema.parse throws; see withParse).
  pinned: z.boolean().default(false),
  // A2 (v0.5.0): optional source-context snippet for a pending record — the slice
  // of the conversation turn the background reviewer drew this memory from. Lets
  // the human审 see WHY it was suggested before confirming, instead of a bare
  // "[kind/imp] content". MUST have .default('') for the same brick reason as
  // pinned — existing records have no such field and open() re-validates all of
  // them. Empty for user-authored / non-review writes.
  sourceContext: z.string().default(''),
  // v0.6.0: which run of the process, and which use within it, last touched this
  // record. A restart resets in-memory counters, so a plain counter cannot tell
  // "used again just now" from "a stale signal from before the restart" -- and
  // getting that wrong means either resurrecting a tombstone forever or dropping
  // a genuine use. MUST have a default: an existing store re-validates every
  // record on open(), and a field without one bricks the whole plugin.
  forgottenGeneration: z.object({
    bootEpoch: z.number().default(0),
    sequence: z.number().default(0),
  }).default({ bootEpoch: 0, sequence: 0 }),
  // v0.6.0: how this write came to exist. `explicit-user-request` is deliberately
  // unreachable from a model tool -- a model that could label its own write as
  // user-requested would walk straight through the process-artifact gate.
  provenance: z.object({
    kind: z.string().default('model-tool'),
    originalKind: z.string().default(''),
    callId: z.string().default(''),
    outcome: z.string().default(''),
  }).default({ kind: 'model-tool', originalKind: '', callId: '', outcome: '' }),
});
...
export const MEMORY_DEFAULTS = {
  ...
  // ── Tiered adjudication (v0.4.0 direction 1; v0.5.0 approvalMode) ────────────
  ...
  // v0.5.0: approvalMode is the product-level autonomy dial (manual/balanced/
  // autonomous). autoConfirmEnabled kept for back-compat (false → manual).
  autoConfirmEnabled: true,
  approvalMode: 'balanced',
  conflictSimilarity: 0.5,
  duplicateSimilarity: 0.82,
  autoMaxImportance: 3,
  // v0.5.0 §3.5 (评审 A1/C1): bound autonomous-tier background-review write volume.
  // Per review turn, at most this many suggestions may AUTO-confirm; the rest fall
  // to pending even if the tier would allow auto. Belt-and-suspenders against a
  // chatty background LLM flooding the store when autonomous is on.
  reviewMaxAutoPerTurn: 5,
  // Hard cap on the PENDING queue (the one region that can be losslessly refused —
  // pending content isn't an asset yet). When a write would exceed this, it is
  // dropped with a warning rather than growing the queue unbounded. This is the
  // real answer to "autonomous + no auto-delete → unbounded growth" (评审 C1):
  // the confirmed budget is char-based (memoryMaxChars) and the pending queue is
  // count-capped here — neither pool can grow without bound.
  maxPendingQueue: 50,
  ...
  // ── Heat / prune / audit (v0.4.2) ───────────────────────────────────────
  // heat = READ-ONLY time-coldness signal (H = 1/(1+λ·Δt)^α). Drives prune
  // candidate ORDER only; NEVER auto-archives. See lib/heat.js. Config items
  // here go through {...MEMORY_DEFAULTS, ...config} merge — NOT storageDomain
  // parse — so no brick risk (unlike record fields).
  heatEnabled: true,
  heatGlobalAlpha: 1.2,
  // per-kind decay λ. preference/decision decay SLOWLY (not zero — "never cools"
  // would itself be bloat; protection is via kind-based prune exclusion, not
  // frozen heat). Bigger λ = cools faster.
  heatKindDecay: {
    preference: 0.0004, decision: 0.0006, fact: 0.0008,
    lesson: 0.001, note: 0.006, todo: 0.01,
  },
  heatSkillDecay: 0.002, // skills have no MEMORY_KINDS; single decay rate.
  // Opt-in idle-refresh of prune candidates (default OFF — no plugin timer).
  // When on: a single unref'd setTimeout re-armed on each write recomputes
  // candidates once the idle window elapses. NEVER setInterval/polling.
  idleRefreshEnabled: false,
  idleMinutes: 5,
  auditMaxRuns: 500,     // prune audit JSONL ring-trim cap.

  // ── Disposal autonomy (v0.5.0 direction 2) ──────────────────────────────────
  // How disposal PROPOSALS are surfaced. Deliberately asymmetric with ingestion:
  // an ingestion mistake is an addition (visible); a disposal mistake is a
  // subtraction (invisible) — so the disposal dial controls WHO PROPOSES, never
  // "skip human confirm".
  //   manual  = nothing proposed automatically; you drive prune from the panel.
  //   suggest = when idle, recompute + surface low-value candidates for review;
  //             STILL zero auto-disposal (heat stays read-only, _reportOverBudget retired).
  // NOTE: the 'tidy' tier (bounded auto soft-delete) is intentionally NOT shipped
  // in v0.5.0 — deferred to v0.6.x (its candidate criteria + tombstone GC haven't
  // converged; see plan §4). Only MEMORY is ever a disposal subject; SKILLS never
  // enter any automatic tier (fold/archive stay manual, conversation-side).
  disposalMode: 'manual',
  tidyMaxPerRun: 5,
  skillProposalMode: 'inherit',
  skillAutoMaxChars: 10000,
  skillMaxChars: 40000,
  approvalPromptEnabled: false,
  approvalPromptMaxPerTurn: 1,
  // A candidate is "cold" only after this many days with zero use on BOTH the
  // injection channel and the access channel (objective, non-heat — 评审 B3/C3/C8).
  disposalMinIdleDays: 30,
};
