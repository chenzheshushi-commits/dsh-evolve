 ...
 * skill-crystallization defaults (SKILL_DEFAULTS) are new to @local/dsh-evolve.
 *
...
/**
 * Skill-crystallization governance (new to @local/dsh-evolve).
 *
...
export const SKILL_DEFAULTS = {
  ...
  // Mirror of Hermes MEMORY.md/USER.md: on each turn/start, inject a small
  ...
  tier1Enabled: true,
  // Char budget for the Tier 1 snapshot. Matches Hermes MEMORY.md's 2200.
  tier1MaxChars: 2200,
  ...
  // Hermes skills. When false (or the LLM call fails/unavailable), falls back to
  // the deterministic string assembly (zero token). Efficiency-consistent with
  // Hermes: single call, on-demand (only at crystallize/refine, maybe 0×/session),
  ...
  // honors the edge-plugin "no setInterval" rule). Mirrors Hermes' ~10.
  reviewEveryTurns: 5,
  ...
  // Hermes curator's DEFAULT_CONSOLIDATE=False. Set convergeSuggest:false to
  // silence the nudge.
