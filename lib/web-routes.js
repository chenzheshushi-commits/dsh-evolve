  ...
  const proposalAction = {
    ...
    handler: async (req,res) => {
      ...
      try{
        ...
        const out=verb==='apply'?await proposalService?.apply?.(id,body?.opId):proposalService?.reject?.(id,body?.opId);
        const status=out?.status==='stale'?409:out?.status==='retry'?202:out?.ok===false?409:200;
