...
export function makeEvolveRoutes(deps) {
  const {
    ...
    listUnclaimed, claimLegacy, proposalService, capabilityService,
  } = deps;
  const guard = (req, res) => {
  ...
  const capabilityMint = {
    kind:'exact',path:`${EVOLVE_API_PREFIX}/capability/mint`,handler:async(req,res)=>{
      ...
      try{const out=capabilityService?.mint?.(body.purpose,body.target,req);return json(res,200,{ok:true,...out})}
  ...
  return [state, action, capabilityMint, proposalList, proposalAction, pruneList, prunePreview, pruneExecute];
}
