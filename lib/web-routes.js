...
export function makeEvolveRoutes(deps) {
  const {
    ...
    listUnclaimed, claimLegacy, proposalService, capabilityService, quarantineService,
  } = deps;
  const guard = (req, res) => {
  ...
  return [state, action, capabilityMint, memoryDiscard, memoryRestoreRejected, proposalList, proposalAction, quarantineList, quarantineAction, pruneList, prunePreview, pruneExecute];
}
