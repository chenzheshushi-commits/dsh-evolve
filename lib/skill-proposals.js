...
function fsyncDir(d){let fd;try{fd=openSync(d,'r');fsyncSync(fd)}finally{if(fd!==undefined)closeSync(fd)}}
...
export class ProposalStore{
 ...
 create({action,targetSkill,body,description='',tag='',sourceIds=[],originals=[],meta={}}){
  ...
  const md=String(body);writeFileSync(join(tmp,'PROPOSAL.md'),md);writeFileSync(join(tmp,'meta.json'),JSON.stringify(proposal,null,2)+'\n');for(const f of ['PROPOSAL.md','meta.json']){const fd=openSync(join(tmp,f),'r');try{fsyncSync(fd)}finally{closeSync(fd)}}fsyncDir(tmp);renameSync(tmp,this.dir(id));fsyncDir(this.root);return proposal
 }
