...
// Windows (and some filesystems) refuse fsync on a handle without write access,
// and always on a directory handle: FlushFileBuffers needs GENERIC_WRITE. Losing
// the durability of a rename must not be silent, but it must not be fatal either
// -- the caller has already written the data. Same contract as op-registry.js.
const FSYNC_SOFT_FAIL = ['EINVAL', 'EACCES', 'EPERM', 'EISDIR', 'ENOTSUP'];
function fsyncDir(d){
  let fd;
  try { fd = openSync(d, 'r'); fsyncSync(fd); }
  catch (e) { if (!FSYNC_SOFT_FAIL.includes(e?.code)) throw e; }
  finally { if (fd !== undefined) try { closeSync(fd); } catch { /* already gone */ } }
}
/** fsync a regular file. Opens 'r+' -- 'r' is rejected by Windows, 'w' truncates. */
function fsyncFile(p){
  let fd;
  try { fd = openSync(p, 'r+'); fsyncSync(fd); }
  catch (e) { if (!FSYNC_SOFT_FAIL.includes(e?.code)) throw e; }
  finally { if (fd !== undefined) try { closeSync(fd); } catch { /* already gone */ } }
}
