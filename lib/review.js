 ...
 *
 * DESIGN (each point is a property this design guarantees):
 ...
 *     adjudicator). The model never writes directly. Safer than a tool-carrying fork
...
const REVIEW_SYSTEM = [
  ...
  '- Do NOT record a project\'s own process artifacts: review verdicts, plan-document',
  '  revisions, scoring notes, "vX.Y.Z revN" findings. The test: a lesson should still',
  '  hold in six months on a different project; if it only means something for one',
  '  review of one version, it belongs in that project\'s documentation, not in memory.',
...
export async function runReview(ctx, exec, { snapshot, cfg, logger }) {
  ...
  try {
    ...
    const messages = [createUserMessage({
      ...
      source: { kind: 'plugin', plugin: 'dsh-evolve', form: 'notice', summary: '对话快照复盘' },
    })];
    const options = {
