...
- **Heat is a read-only ordering signal.** Each memory gets a power-law coldness score `H = 1 / (1 + λ·Δt)^α`. Time basis is `accessedAt || createdAt` — **never `updatedAt`** (merging / refining bumps `updatedAt` but that is not "access", and a common scoring bug). Heat only orders prune candidates; it never archives anything automatically.
